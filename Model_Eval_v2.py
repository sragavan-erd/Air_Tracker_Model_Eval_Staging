...
mDATE = cDATE.replace(minute=0,
                      second=0, microsecond=0) 
                      

print(mDATE)

mwm = get_mesowest_radius(mDATE, '40.65,-112.0', '20',
                           variables=hrrr_vars, extra='', set_num=0, verbose=True) 

if mwm =='ERROR':
    #retry?
    raise FileNotFoundError('Error fetching MesoWest data. Exiting the program...') #Error catching
...
bucket = GoogleCloudStorageBucket('air-tracker-edf-stilt-meteorology-prod') #This has the HRRR SLC Subset Data
...
uv = xr.open_dataset('/share/air-tracker-edf/src/hrrr-uncertainty/hrrr_wind.nc') #Using the xarray library to open the NetCDF file 
...
with open(f'/share/air-tracker-edf/src/hrrr-uncertainty/ModelEvalOutput/ME{dat2}.json', 'w') as outfile:
